...
h:  1 0 0 2 3 
    ...
    một node sẽ quản lí từ l -> r
    trong node có 1 biến size là số lượng phần tử chưa xóa
    trong node có 1 trường max là max những phần tử chưa xóa

    Q 1 2
    l = 2, r = 5

    nhóm x đến y tìm phần tử nhỏ nhất mang nhóm x và phần tử lớn nhất mang nhóm y
    ...
    seg 1
    mình sẽ cần 1 cái cây 2 truy vấn 
    truy vấn 1 là tăng d[i] lên 1 đơn vị
    truy vấn 2 là tổng từ d1 đến di là để biết i thuộc nhóm nào
    ...
    ll erase(id, l, r, h){
        ...
        // l -> mid, mid + 1 -> r
        // st[id].size 
        if(st[id << 1].size < h) pos = get(id << 1 | 1, mid + 1, r, h - st[id << 1].size);
        else pos = get(id << 1, l, mid, h);
        st[id].size = st[id << 1].size + st[id << 1 | 1].size;
        st[id].max = max(st[id<<1].max, st[id<<1|1].max);
        return pos;
    }
    ll get_max(id, l, r, u, v){
        if(v < l or r < u) return -1;
        if(u <= l and r <= v) return st[id].max;
        ll mid = (l + r) >> 1;
        return max(get_max(phia ben trai), get_max(phia ben phai));
    }
*/