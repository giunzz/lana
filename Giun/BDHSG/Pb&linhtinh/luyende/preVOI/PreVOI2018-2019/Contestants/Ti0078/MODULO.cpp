...
void Get1()
{
    ...
    if(chk1[val].v[1]==0)chk1[val]=res;
}
...
void Get2()
{
    ...
    if(chk2[val].v[n]==0)
    {
        ...
        if(val<=k)
        {
            if(chk1[k-val].v[1]!=0)
            {
                for(int j=n;j>mid;--j)cout<<chk2[val].v[j];
                for(int j=mid;j>0;--j)cout<<chk1[k-val].v[j];
                exit(0);
            }
        }
        ...
        {
            if(chk1[p+k-val].v[1]!=0)
            {
                for(int j=n;j>mid;--j)cout<<chk2[val].v[j];
                for(int j=mid;j>0;--j)cout<<chk1[p+k-val].v[j];
                exit(0);
            }
        }
    }
