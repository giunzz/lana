...
int n, k, a[maxN], b[maxN], f[maxN][maxN], ans = 0;

vector <int> trace[maxN][maxN], anstrace;
        ...
        for (int j=1; j<=n; ++j)
            if (a[i] == b[j])
            {
                ...
                for (pair <int, int> t: v)
                    if (t.first < i && t.second < j && abs(a[t.first] - a[i] <= k))
                    {
                        if (f[i][j] == f[t.first][t.second])
                            trace[i][j] = Max(trace[i][j], trace[t.first][t.second]);
                        if (f[i][j] < f[t.first][t.second])
                        {
                            f[i][j] = f[t.first][t.second];
                            trace[i][j] = trace[t.first][t.second];
                        }
                    }
                ++f[i][j];
                trace[i][j].push_back(a[i]);
                if (ans == f[i][j])
                    anstrace = Max(anstrace, trace[i][j]);
                if (ans < f[i][j])
                {
                    ans = f[i][j];
                    anstrace = trace[i][j];
