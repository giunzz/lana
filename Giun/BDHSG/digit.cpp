...
ll sol_dp(bool equal, bool first0, int pos, ll div, int dig){
    ll &cur = dp[equal][first0][pos][div][dig], npos = pos - 1, div10 = (div * 10) % x;
    bool nequal = equal;
    if(~cur) return cur;
    if(!pos) return cur = ((!div && !first0) || (first0 && al[0]));
    cur = 0;
    if(first0) cur += sol_dp(0, first0, npos, 0, 0);
    for(int ndig : dg){
        ...
        cur += sol_dp(nequal, 0, npos, (div10 + ndig) % x, ndig);
    }   
    return cur;
}
...
ll pre_dp(ll num){
    ...
    return sol_dp(1, 1, dg_n.size(), 0, 0);
}
