...
bool check(ll x1, ll y1, ll x2, ll y2){
    if(y1 < 1 || y1 > m || y2 < 1 || y2 > m) return 0;
    if(x1 < 1 || x1 > n || x2 < 1 || x2 > n) return 0;
...
int main(){
    ...
    lp(i, 1, ((m + n) >> 1) - 1) {
        lp(x1, 1, n) lpd(x2, n, 1){
            ll y1 = i + 1 - x1, y2 = m + n - x2 + 1 - i, cur = dp[i & 1][x1][x2];
            if(y1 < 1 || y1 > m || y2 < 1 || y2 > m) continue;
            // cerr << i << "     " << x1 << ' ' << y1 << "      " << x2 << ' ' << y2 << "\n";
            // dp[i + 1][x1 + 1][x2]dc
            ll nstat = (i + 1) & 1;
            if(check(x1 + 1, y1, x2 - 1, y2)) (dp[nstat][x1 + 1][x2 - 1] += cur) %= MOD;
            if(check(x1, y1 + 1, x2 - 1, y2)) (dp[nstat][x1][x2 - 1] += cur) %= MOD;
            if(check(x1 + 1, y1, x2, y2 - 1)) (dp[nstat][x1 + 1][x2] += cur) %= MOD;
            if(check(x1, y1 + 1, x2, y2 - 1)) (dp[nstat][x1][x2] += cur) %= MOD;
            dp[i & 1][x1][x2] = 0;
        }// m - y2 + n - x2 + 1 = sl
    }
    ll sl = (n + m) / 2, ans = 0;
    if((n + m) & 1){
        lp(i, 1, n) (ans += dp[sl & 1][i][i] + dp[sl & 1][i][i + 1]) %= MOD;
    } else{
        lp(i, 1, n) (ans += dp[sl & 1][i][i]) %= MOD;
    }
    // lp(i, 1, n) cerr << i << "      " << dp[sl][i][i] << "   " << dp[sl][i][i + 1] << '\n';
    cout << ans;
}
