#include <bits/stdc++.h>
#include "source_done.h"
using namespace std;
const long long maxtime = 1000, testdau = 0, testcuoi = 5; //ms
 
ifstream test;
ifstream code;
ofstream fi;
char name[225] = "pathk", fol[225] = "pathk\\test0\\"; 
...
void cham(){
    ...
    strcpy(link, "");
    if (!test.is_open()){
        cerr << ": LOI MO FILE TEST";
        return;
    }
    if (!code.is_open()){
        cerr << ": LOI MO FILE CODE";
        return;
    }
    ...
    while (test >> q1){
        ...
        ++COUNT_ANS;
        if (q1 != q2) {
            cerr << ": WRONG " << q1 << " " << q2 << " vi tri sai: " << COUNT_ANS;
            return;
        }
    }
    q1 = 0, q2 = 0;
    while (test >> x){q1 = 1;}
    while (code >> x){q2 = 1;}
    if (q1 == q2) cerr << ((timerunct <= maxtime) ? ": ACCEPT " : ": TLE ") << COUNT_ANS << " time: " << timerunct << " ms";
    else cerr << ": ERROR ";
}
