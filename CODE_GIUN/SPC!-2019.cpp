...
using namespace std;
ll test , dp_cnt[17][10][2]={0}; // pos , num  , ok  ,
ll dp_sol[17][10][2]={0};
vector<ll> digit;
...
ll sol(ll pos, ll num , bool ok)
{
    ...
    for (int nnum =  0 ; nnum <= 9 ; nnum++)
    {
        if (ok)
        {
            if (nnum <  digit[npos]) res += sol (npos,nnum,0) + num * cnt(npos,nnum,0);
            else if (nnum == digit[npos]) res += sol(npos,nnum,1) + num * cnt(npos,nnum,1);
        }
        else res += sol(npos , nnum ,ok) + num * cnt(npos , nnum ,ok);
    }
...
ll cal (ll x)
{
    ...
    while (digit.size() < 17) digit.push_back(0);
