...
int main()
{
    ...
    f[1] = a[1] , f[2] = a[1] + a[2] , f[3] = max({a[1]+a[2], a[2]+a[3], a[1]+a[3]});
    for (int i = 4 ; i <= n ; i++) 
    ...
    while ( i > 2 )
    {
        ...
        else if (f[i]  == f[i-3] + a[i] + a[i-1] ) ans.push_back(i) , ans.push_back(i-1), i-=3;
    }
    if(i == 1) ans.push_back(i);
    else if(i == 2) ans.push_back(i), ans.push_back(i - 1);
